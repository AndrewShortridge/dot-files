...
function M.next_date(from_date_str, rule)
  ...
  if rule.type == "weekday" then
    ...
    if wday == 0 then
      t = t + date_utils.SECS_PER_DAY -- Sun -> Mon
    elseif wday == 6 then
      t = t + 2 * date_utils.SECS_PER_DAY -- Sat -> Mon
    end
    return os.date("%Y-%m-%d", t)
  end
...
function M.handle_recurrence(line_nr)
  ...
  -- Parse task inline fields via centralized parser
  ...
  local fields = vault_index.parse_task_fields(task_text)
  if not fields.repeat_rule then
    return false
  end

  local rule = M.parse_rule(fields.repeat_rule)
  if not rule then
    return false
  end

  -- Determine base date: use parsed due date if present, otherwise today
  local due_date = fields.due
  local base_date = due_date or engine.today()
