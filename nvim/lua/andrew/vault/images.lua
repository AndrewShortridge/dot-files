...
function M.paste_image()
  ...
  elseif vim.fn.executable("xsel") == 1 then
    notify.warn("xsel does not support image paste")
    return
  else
    notify.warn("no clipboard tool found (need xclip or wl-paste)")
    return
  end
  ...
  vim.system(cmd, {}, function(result)
    vim.schedule(function()
      ...
      if result.code ~= 0 or not stat or stat.size == 0 then
        notify.error("failed to paste image from clipboard")
...
function M.setup()
  ...
  palette.register_command("VaultPasteImage", "Vault: paste clipboard image", "Edit", function()
    M.paste_image()
  end, "<leader>vp")
end
