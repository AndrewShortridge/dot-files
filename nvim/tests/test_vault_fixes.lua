-- Test suite for vault bug fixes
-- Run with: nvim --headless -u NONE -l tests/test_vault_fixes.lua

local passed = 0
local failed = 0
local errors = {}

local function test(name, fn)
  local ok, err = pcall(fn)
  if ok then
    passed = passed + 1
    print("  PASS: " .. name)
  else
    failed = failed + 1
    table.insert(errors, { name = name, err = tostring(err) })
    print("  FAIL: " .. name .. " -> " .. tostring(err))
  end
end

local function assert_eq(got, expected, msg)
  if got ~= expected then
    error((msg or "") .. " expected: " .. vim.inspect(expected) .. ", got: " .. vim.inspect(got))
  end
end

local function assert_true(val, msg)
  if not val then
    error((msg or "assertion failed") .. " (got falsy)")
  end
end

local function assert_false(val, msg)
  if val then
    error((msg or "assertion failed") .. " (got truthy)")
  end
end

local function assert_match(str, pattern, msg)
  if not str:match(pattern) then
    error((msg or "") .. " string '" .. str .. "' does not match pattern '" .. pattern .. "'")
  end
end

-- ============================================================================
-- Setup: create temp vault structure for testing
-- ============================================================================
local tmp_vault = vim.fn.tempname() .. "/test-vault"
vim.fn.mkdir(tmp_vault .. "/Projects/Alpha/Tasks", "p")
vim.fn.mkdir(tmp_vault .. "/Projects/Beta", "p")
vim.fn.mkdir(tmp_vault .. "/Archive", "p")
vim.fn.mkdir(tmp_vault .. "/Domains/Physics", "p")
vim.fn.mkdir(tmp_vault .. "/Log", "p")
vim.fn.mkdir(tmp_vault .. "/Areas/Career", "p")
vim.fn.mkdir(tmp_vault .. "/.obsidian", "p")

-- Write test files
local function write_file(rel, content)
  local path = tmp_vault .. "/" .. rel
  local dir = vim.fn.fnamemodify(path, ":h")
  vim.fn.mkdir(dir, "p")
  local f = io.open(path, "w")
  f:write(content)
  f:close()
end

write_file("Projects/Alpha/Dashboard.md", [[---
type: project
tags:
  - physics
  - simulation
  - active
  - high-priority
  - review-needed
  - quarterly
  - funded
  - collaborative
  - experimental
  - published
  - peer-reviewed
  - conference
  - journal
  - grant
  - nist
  - doe
  - nsf
  - darpa
  - army-research
  - navy-research
  - air-force
  - space-force
  - dod
  - nih
  - cdc
  - fda
  - epa
  - noaa
  - nasa
  - usgs
  - usda
aliases: [Alpha Project, Project A]
status: active
modified: 2026-01-15T10:30:00
created: 2025-06-01T08:00:00
---
# Alpha Project Dashboard

## Overview
This is the Alpha project.

## Status
Currently active.
]])

write_file("Projects/Alpha/Tasks/Task1.md", [[---
type: task
status: open
priority: 1
due: 2026-03-01
created: 2026-02-01T10:00:00
modified: 2026-02-20T15:30:00
---
# Fix the simulation parameters

- [ ] Review LAMMPS config [due:: 2026-03-01]
- [x] Update boundary conditions [completion:: 2026-02-15]
- [/] Run convergence test
- [-] Old approach cancelled
- [>] Deferred to next sprint
]])

write_file("Archive/Alpha.md", [[---
type: archive
status: archived
modified: 2025-01-01T00:00:00
---
# Alpha (Archived)

This is an older archived note also named Alpha.
]])

write_file("Projects/Beta/Dashboard.md", [==[---
type: project
tags: [ml, deep-learning]
status: active
modified: 2026-02-01T10:00:00
created: 2026-02-18T14:00:00
---
# Beta Project

See also [[Alpha|Alpha Project]] and [[Alpha#Overview]].
References: [[Dashboard]] and [[NonExistent]].
]==])

write_file("Domains/Physics/Concepts.md", [==[---
type: concept
tags: [physics, mechanics]
modified: 2026-01-20T09:00:00
---
# Physics Concepts

## Newton's Laws

Basic mechanics overview.

## Thermodynamics

Heat and energy.

```python
# This is code with [[fake link]] inside
x = 1
```

Inline code: `[[not a link]]` should be ignored.

Some text with #physics and #mechanics tags.
]==])

write_file("Log/2026-02-21.md", [[---
type: log
modified: 2026-02-21T08:00:00
---
# Daily Log 2026-02-21

- [x] Morning standup
- [ ] Review PRs
]])

-- File with long frontmatter (> 30 lines)
local long_fm_lines = { "---", "type: literature" }
for i = 1, 40 do
  table.insert(long_fm_lines, "field_" .. i .. ": value_" .. i)
end
table.insert(long_fm_lines, "modified: 2025-01-01T00:00:00")
table.insert(long_fm_lines, "---")
table.insert(long_fm_lines, "# Long Frontmatter Note")
write_file("long_fm.md", table.concat(long_fm_lines, "\n"))

-- ============================================================================
print("\n=== 1. ENGINE: Coroutine safety (vim.schedule wrap) ===")
-- ============================================================================

test("engine.input wraps callback in vim.schedule", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/engine.lua"), "r"):read("*a")
  -- Check that vim.ui.input callback uses vim.schedule
  assert_true(
    src:match("vim%.ui%.input%(opts, function%(value%)\n%s+vim%.schedule%(function%(%)"),
    "vim.ui.input callback should be wrapped in vim.schedule"
  )
end)

test("engine.select wraps callback in vim.schedule", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/engine.lua"), "r"):read("*a")
  assert_true(
    src:match("vim%.ui%.select%(items, opts, function%(choice%)\n%s+vim%.schedule%(function%(%)"),
    "vim.ui.select callback should be wrapped in vim.schedule"
  )
end)

test("engine.write_note includes OS error in notification", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/engine.lua"), "r"):read("*a")
  assert_true(
    src:match('io_err or "unknown error"'),
    "io.open failure should include the OS error message"
  )
end)

-- ============================================================================
print("\n=== 2. WIKILINKS: Cache stores lists, proximity resolution ===")
-- ============================================================================

-- Directly test the cache data structure by simulating what build_cache does
test("wikilink cache stores multiple paths per basename", function()
  -- Simulate the new cache logic with two files sharing the same basename "Alpha"
  local cache = {}
  local test_files = {
    tmp_vault .. "/Projects/Alpha/Alpha.md",
    tmp_vault .. "/Archive/Alpha.md",
  }
  for _, path in ipairs(test_files) do
    local basename = vim.fn.fnamemodify(path, ":t:r"):lower()
    if not cache[basename] then
      cache[basename] = {}
    end
    table.insert(cache[basename], path)
  end

  assert_eq(type(cache["alpha"]), "table", "cache['alpha'] should be a table")
  assert_eq(#cache["alpha"], 2, "cache['alpha'] should have 2 entries for same basename")
end)

test("proximity resolution picks closest directory", function()
  -- Simulate resolve_link with proximity logic
  local paths = {
    tmp_vault .. "/Projects/Alpha/Dashboard.md",
    tmp_vault .. "/Archive/Alpha.md",
  }
  local current_dir = tmp_vault .. "/Projects/Alpha/Tasks"

  local best_path = paths[1]
  local best_score = math.huge
  for _, path in ipairs(paths) do
    local dir = vim.fn.fnamemodify(path, ":h")
    local common = 0
    for i = 1, math.min(#dir, #current_dir) do
      if dir:sub(i, i) == current_dir:sub(i, i) then
        common = common + 1
      else
        break
      end
    end
    local score = (#dir - common) + (#current_dir - common)
    if score < best_score then
      best_score = score
      best_path = path
    end
  end

  assert_match(best_path, "Projects/Alpha", "Should pick Projects/Alpha/Dashboard.md when in Projects/Alpha/Tasks")
end)

test("proximity resolution picks Archive when closer", function()
  local paths = {
    tmp_vault .. "/Projects/Alpha/Dashboard.md",
    tmp_vault .. "/Archive/Alpha.md",
  }
  local current_dir = tmp_vault .. "/Archive"

  local best_path = paths[1]
  local best_score = math.huge
  for _, path in ipairs(paths) do
    local dir = vim.fn.fnamemodify(path, ":h")
    local common = 0
    for i = 1, math.min(#dir, #current_dir) do
      if dir:sub(i, i) == current_dir:sub(i, i) then
        common = common + 1
      else
        break
      end
    end
    local score = (#dir - common) + (#current_dir - common)
    if score < best_score then
      best_score = score
      best_path = path
    end
  end

  assert_match(best_path, "Archive/Alpha", "Should pick Archive/Alpha.md when in Archive/")
end)

-- ============================================================================
print("\n=== 3. WIKILINKS: New note created in current buffer dir ===")
-- ============================================================================

test("new note location logic uses buffer dir when inside vault", function()
  local vault_path = tmp_vault
  local buf_dir = tmp_vault .. "/Projects/Alpha/Tasks"
  local link = "NewNote"

  local new_path
  if vim.startswith(buf_dir, vault_path) then
    new_path = buf_dir .. "/" .. link .. ".md"
  else
    new_path = vault_path .. "/" .. link .. ".md"
  end

  assert_eq(new_path, tmp_vault .. "/Projects/Alpha/Tasks/NewNote.md",
    "New note should be in buffer's directory")
end)

test("new note location falls back to vault root when outside vault", function()
  local vault_path = tmp_vault
  local buf_dir = "/tmp/somewhere-else"
  local link = "NewNote"

  local new_path
  if vim.startswith(buf_dir, vault_path) then
    new_path = buf_dir .. "/" .. link .. ".md"
  else
    new_path = vault_path .. "/" .. link .. ".md"
  end

  assert_eq(new_path, tmp_vault .. "/NewNote.md",
    "New note should fall back to vault root")
end)

-- ============================================================================
print("\n=== 4. BACKLINKS: Regex matches aliased and heading links ===")
-- ============================================================================

-- Test ripgrep regex patterns (simulate matching)
local function rg_escape(str)
  return str:gsub("([%(%)%.%%%+%-%*%?%[%]%^%${}|\\])", "\\%1")
end

-- Convert the ripgrep regex to Lua pattern for testing
-- The backlink regex: \[\[name([#|][^\]]*)?]]
test("backlink regex matches plain [[Note]]", function()
  local name = "Alpha"
  local text = "See [[Alpha]] for details"
  -- The rg regex: \[\[Alpha([#|][^\]]*)?]]
  -- In Lua, test equivalent:
  assert_true(text:match("%[%[" .. name .. "%]%]") ~= nil, "Should match [[Alpha]]")
end)

test("backlink regex matches [[Note|alias]]", function()
  local text = "See [[Alpha|Alpha Project]] for details"
  -- Old regex would need exact ]] after name - would fail
  -- New regex allows [#|] followed by anything before ]]
  -- Test: does "Alpha" appear before a | or # or ]] ?
  local pattern = "%[%[Alpha[#|%]]"
  assert_true(text:match(pattern) ~= nil, "Should match [[Alpha|...]]")
end)

test("backlink regex matches [[Note#heading]]", function()
  local text = "See [[Alpha#Overview]] for details"
  local pattern = "%[%[Alpha[#|%]]"
  assert_true(text:match(pattern) ~= nil, "Should match [[Alpha#heading]]")
end)

test("backlink regex matches [[Note#heading|alias]]", function()
  local text = "See [[Alpha#Overview|Alpha Overview]] for details"
  local pattern = "%[%[Alpha[#|%]]"
  assert_true(text:match(pattern) ~= nil, "Should match [[Alpha#heading|alias]]")
end)

-- ============================================================================
print("\n=== 5. BACKLINKS: Heading backlinks use raw text ===")
-- ============================================================================

test("heading backlinks use index-based find_link_lines with heading filter", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/backlinks.lua"), "r"):read("*a")
  -- heading_backlinks uses find_link_lines with the heading parameter for filtering
  assert_true(
    src:match('find_link_lines%(.-heading%)'),
    "heading_backlinks should use find_link_lines with heading filter"
  )
  -- Should not have rg fallback functions anymore
  assert_false(
    src:match('_backlinks_rg'),
    "should not have rg fallback function"
  )
end)

-- ============================================================================
print("\n=== 6. SEARCH: All notes glob is recursive ===")
-- ============================================================================

test("search_filtered All notes uses **/*.md glob", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/config.lua"), "r"):read("*a")
  assert_true(
    src:match('"All notes",%s*glob%s*=%s*"%*%*/%*%.md"'),
    "All notes should use **/*.md glob for recursive search"
  )
  assert_false(
    src:match('"All notes",%s*glob%s*=%s*"%*%.md"'),
    "All notes should NOT use *.md (non-recursive)"
  )
end)

-- ============================================================================
print("\n=== 7. SEARCH: type regex handles whitespace ===")
-- ============================================================================

test("search_by_type uses \\s+ for flexible whitespace", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/search.lua"), "r"):read("*a")
  assert_true(
    src:match("\\s%+"),
    "search_by_type should use \\s+ for whitespace matching"
  )
end)

-- ============================================================================
print("\n=== 8. COMPLETION: Frontmatter parsing has no 30-line limit ===")
-- ============================================================================

test("completion parse_frontmatter uses while loop, not limited for loop", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/completion.lua"), "r"):read("*a")
  assert_false(
    src:match("for _ = 1, 30 do"),
    "Should not have for _ = 1, 30 limit"
  )
  assert_true(
    src:match("while true do"),
    "Should use while true do for unlimited frontmatter reading"
  )
end)

test("completion parse_frontmatter reads all fields from long frontmatter", function()
  -- Directly test the parse_frontmatter logic
  local path = tmp_vault .. "/Projects/Alpha/Dashboard.md"
  local f = io.open(path, "r")
  assert_true(f ~= nil, "Test file should exist")

  local first = f:read("*l")
  assert_eq(first, "---", "First line should be ---")

  local fm = {}
  local cur_key = nil
  local cur_list = nil

  while true do
    local line = f:read("*l")
    if not line or line == "---" then break end

    local list_item = line:match("^%s+-%s+(.+)$")
    if list_item and cur_key then
      if not cur_list then cur_list = {} end
      cur_list[#cur_list + 1] = list_item
      fm[cur_key] = table.concat(cur_list, ", ")
    else
      local key, val = line:match("^(%w[%w_-]*):%s*(.*)$")
      if key then
        cur_key = key
        cur_list = nil
        if val and val ~= "" then
          val = val:gsub("^%[", ""):gsub("%]$", ""):gsub("^'", ""):gsub("'$", ""):gsub('^"', ""):gsub('"$', "")
          fm[key] = val
        end
      end
    end
  end
  f:close()

  assert_eq(fm.type, "project", "Should parse type field")
  assert_eq(fm.status, "active", "Should parse status field")
  assert_true(fm.tags ~= nil, "Should parse tags")
  assert_true(fm.aliases ~= nil, "Should parse aliases")
end)

test("completion parse_frontmatter handles long frontmatter (40+ fields)", function()
  local path = tmp_vault .. "/long_fm.md"
  local f = io.open(path, "r")
  assert_true(f ~= nil, "Long frontmatter test file should exist")

  local first = f:read("*l")
  assert_eq(first, "---", "First line should be ---")

  local fm = {}
  local cur_key = nil
  local cur_list = nil

  while true do
    local line = f:read("*l")
    if not line or line == "---" then break end

    local list_item = line:match("^%s+-%s+(.+)$")
    if list_item and cur_key then
      if not cur_list then cur_list = {} end
      cur_list[#cur_list + 1] = list_item
      fm[cur_key] = table.concat(cur_list, ", ")
    else
      local key, val = line:match("^(%w[%w_-]*):%s*(.*)$")
      if key then
        cur_key = key
        cur_list = nil
        if val and val ~= "" then
          fm[key] = val
        end
      end
    end
  end
  f:close()

  assert_eq(fm.type, "literature", "Should parse type from long frontmatter")
  assert_eq(fm.field_1, "value_1", "Should parse field_1")
  assert_eq(fm.field_30, "value_30", "Should parse field_30 (was previous limit)")
  assert_eq(fm.field_40, "value_40", "Should parse field_40 (beyond old limit)")
  assert_true(fm.modified ~= nil, "Should parse modified field after 40+ fields")
end)

-- ============================================================================
print("\n=== 9. COMPLETION_BASE: Race condition fix (generation counter) ===")
-- ============================================================================

test("completion has build_generation counter", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/completion_base.lua"), "r"):read("*a")
  assert_true(src:match("build_generation"), "Should have build_generation variable")
end)

test("completion invalidate increments generation", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/completion_base.lua"), "r"):read("*a")
  assert_true(
    src:match("build_generation = build_generation %+ 1"),
    "invalidate() should increment build_generation"
  )
end)

test("completion build_items_async captures and checks generation", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/completion_base.lua"), "r"):read("*a")
  assert_true(
    src:match("local gen = build_generation"),
    "build_items_async should capture generation"
  )
  assert_true(
    src:match("gen ~= build_generation"),
    "build_items_async should check generation before writing results"
  )
end)

-- ============================================================================
print("\n=== 10. FRONTMATTER: Line limit increased ===")
-- ============================================================================

test("frontmatter reads up to 200 lines, not 30", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/frontmatter.lua"), "r"):read("*a")
  assert_false(
    src:match("nvim_buf_get_lines%(ev%.buf, 0, 30,"),
    "Should not hardcode 30-line limit"
  )
  assert_true(
    src:match("math%.min%(line_count, config%.frontmatter%.max_scan_lines%)"),
    "Should read up to max_scan_lines (200) via config"
  )
end)

-- ============================================================================
print("\n=== 11. EXECUTOR: Pattern injection fix in contains_value ===")
-- ============================================================================

test("contains_value uses plain string find, not pattern match", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/query/executor.lua"), "r"):read("*a")
  assert_true(
    src:match("v%.path:find%(b, 1, true%)"),
    "Should use string.find with plain=true"
  )
  assert_false(
    src:match("v%.path:match%(b%)"),
    "Should NOT use string.match (pattern injection risk)"
  )
end)

test("contains_value plain find works with special characters", function()
  -- Simulate the fixed contains_value behavior
  local link_path = "Projects/Alpha.Beta/Dashboard"

  -- Old behavior: :match(b) where b has "." which is a wildcard
  local bad_result = link_path:match("Alpha.Beta") -- matches "AlphaXBeta" too
  assert_true(bad_result ~= nil, "Lua match treats . as wildcard (old bug)")

  -- New behavior: :find(b, 1, true) does plain substring match
  local good_result = link_path:find("Alpha.Beta", 1, true)
  assert_true(good_result ~= nil, "Plain find matches exact substring")

  -- The key fix: special chars in b don't cause unexpected matches
  local no_match = ("Projects/AlphaXBeta/Dashboard"):find("Alpha.Beta", 1, true)
  assert_true(no_match == nil, "Plain find should NOT match AlphaXBeta with Alpha.Beta")
end)

test("contains_value plain find rejects Lua pattern metacharacters", function()
  -- Patterns that would crash or misbehave with :match()
  local test_cases = {
    { path = "foo(bar)", search = "(bar)" },
    { path = "foo[1]", search = "[1]" },
    { path = "100%done", search = "%done" },
    { path = "a+b", search = "a+b" },
  }

  for _, tc in ipairs(test_cases) do
    -- Old behavior would crash or match incorrectly
    local ok, _ = pcall(function()
      return tc.path:match(tc.search)
    end)
    -- New behavior: plain find always works
    local result = tc.path:find(tc.search, 1, true)
    assert_true(result ~= nil, "Plain find should work for: " .. tc.search)
  end
end)

-- ============================================================================
print("\n=== 12. EXECUTOR: LIMIT applied before GROUP BY ===")
-- ============================================================================

test("executor applies LIMIT before GROUP BY in pipeline", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/query/executor.lua"), "r"):read("*a")

  -- Find the pipeline step comments (numbered steps in the execute function)
  local limit_pos = src:find("%-%- 5%. LIMIT")
  local group_pos = src:find("%-%- 6%. GROUP BY")

  assert_true(limit_pos ~= nil, "Should have step 5 LIMIT comment")
  assert_true(group_pos ~= nil, "Should have step 6 GROUP BY comment")
  assert_true(limit_pos < group_pos, "LIMIT (step 5) should come before GROUP BY (step 6)")
end)

test("executor LIMIT is not conditional on groups", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/query/executor.lua"), "r"):read("*a")
  assert_false(
    src:match("if not groups then\n%s+pages = apply_limit"),
    "LIMIT should NOT be conditional on groups"
  )
end)

-- Functional test: LIMIT + GROUP BY
test("LIMIT caps total results even with GROUP BY", function()
  local types = require("andrew.vault.query.types")
  local executor = require("andrew.vault.query.executor")

  -- Create a minimal index mock
  local mock_index = {
    all_pages = function()
      local pages = {}
      for i = 1, 10 do
        local group = (i <= 5) and "A" or "B"
        pages[i] = {
          file = {
            name = "note" .. i,
            path = "note" .. i .. ".md",
            link = types.Link.new("note" .. i, "note" .. i, false),
            tags = {},
          },
          group_field = group,
        }
      end
      return pages
    end,
    resolve_source = function(self, node)
      return self:all_pages()
    end,
    current_page = function() return nil end,
  }

  -- Query: TABLE group_field FROM "" GROUP BY group_field LIMIT 5
  local ast = {
    type = "TABLE",
    fields = { { expr = { type = "field", path = { "group_field" } }, alias = nil } },
    group_by = { expr = { type = "field", path = { "group_field" } } },
    limit = 5,
    without_id = false,
  }

  local results = executor.execute(ast, mock_index, "")

  -- Count total rows across all groups
  local total_rows = 0
  for _, result in ipairs(results) do
    if result.rows then
      total_rows = total_rows + #result.rows
    end
  end

  assert_true(total_rows <= 5, "LIMIT 5 with GROUP BY should cap total to 5, got " .. total_rows)
end)

-- ============================================================================
print("\n=== 13. Backtick stripping in task metadata extraction ===")
-- ============================================================================

local parse_task_fields = require("andrew.vault.vault_index").parse_task_fields

test("parse_task_fields ignores [due::] inside single backticks", function()
  local meta = parse_task_fields("Check `[due:: 2026-01-01]` value")
  assert_eq(meta.due, nil, "due inside backticks should be nil")
end)

test("parse_task_fields extracts real field, ignores backtick-enclosed one", function()
  local meta = parse_task_fields("Run `[due:: fake]` then [due:: 2026-03-02]")
  assert_eq(meta.due, "2026-03-02", "due should be the real value outside backticks")
end)

test("parse_task_fields handles double-backtick code spans", function()
  local meta = parse_task_fields("Use ``[priority:: 1]`` syntax for fields [priority:: 3]")
  assert_eq(meta.priority, 3, "priority should be 3 (double-backtick span stripped)")
end)

test("parse_task_fields ignores (repeat::) inside backticks", function()
  local meta = parse_task_fields("Example `(repeat:: every day)` note")
  assert_eq(meta.repeat_rule, nil, "repeat inside backticks should be nil")
end)

test("parse_task_fields handles double-backtick wrapping single backtick", function()
  local meta = parse_task_fields("Show `` ` `` char [due:: 2026-04-01]")
  assert_eq(meta.due, "2026-04-01", "due outside double-backtick span should parse")
end)

test("parse_task_fields treats unmatched backtick as literal", function()
  local meta = parse_task_fields("This has a `stray backtick and [due:: 2026-05-01]")
  assert_eq(meta.due, "2026-05-01", "due after unmatched backtick should parse (literal text)")
end)

test("parse_task_fields strips multiple code spans on same line", function()
  local meta = parse_task_fields("Use `[due:: x]` and `(priority:: 9)` syntax [scheduled:: 2026-06-01]")
  assert_eq(meta.due, nil, "due inside backticks should be nil")
  assert_eq(meta.priority, nil, "priority inside backticks should be nil")
  assert_eq(meta.scheduled, "2026-06-01", "scheduled outside backticks should parse")
end)

test("parse_task_fields works normally with no backticks", function()
  local meta = parse_task_fields("Important task [due:: 2026-02-15] [priority:: 1]")
  assert_eq(meta.due, "2026-02-15", "due should parse normally")
  assert_eq(meta.priority, 1, "priority should parse normally")
end)

-- ============================================================================
print("\n=== 14. INDEX: Date parsing for YYYY-MM-DDTHH:MM:SS ===")
-- ============================================================================

test("_parse_scalar parses YYYY-MM-DD date", function()
  local Index = require("andrew.vault.query.index").Index
  local idx = Index.new(tmp_vault)

  local result = idx:_parse_scalar("2026-02-18")
  assert_true(type(result) == "table", "Should parse to a Date table")
  assert_eq(result.year, 2026, "Year should be 2026")
  assert_eq(result.month, 2, "Month should be 2")
  assert_eq(result.day, 18, "Day should be 18")
end)

test("_parse_scalar parses YYYY-MM-DDTHH:MM:SS datetime", function()
  local Index = require("andrew.vault.query.index").Index
  local idx = Index.new(tmp_vault)

  local result = idx:_parse_scalar("2026-02-18T10:30:00")
  assert_true(type(result) == "table", "Should parse to a Date table")
  assert_eq(result.year, 2026, "Year should be 2026")
  assert_eq(result.month, 2, "Month should be 2")
  assert_eq(result.day, 18, "Day should be 18")
  assert_eq(result.hour, 10, "Hour should be 10")
  assert_eq(result.min, 30, "Minute should be 30")
end)

test("_parse_scalar does not parse random strings as dates", function()
  local Index = require("andrew.vault.query.index").Index
  local idx = Index.new(tmp_vault)

  local result = idx:_parse_scalar("hello world")
  assert_eq(type(result), "string", "Plain strings should stay as strings")
  assert_eq(result, "hello world", "Should return original string")
end)

test("_parse_scalar does not parse numbers as dates", function()
  local Index = require("andrew.vault.query.index").Index
  local idx = Index.new(tmp_vault)

  local result = idx:_parse_scalar("42")
  assert_eq(type(result), "number", "Should parse as number")
  assert_eq(result, 42, "Should be 42")
end)

test("_parse_scalar parses date with space separator (YYYY-MM-DD HH:MM:SS)", function()
  local Index = require("andrew.vault.query.index").Index
  local idx = Index.new(tmp_vault)

  -- The Date.parse function handles ISO format, let's check if our gate lets it through
  -- text:match("^%d%d%d%d%-%d%d%-%d%d[T ]") should match space-separated datetimes
  local text = "2026-02-18T14:00:00"
  assert_true(text:match("^%d%d%d%d%-%d%d%-%d%d[T ]") ~= nil, "Should match T-separated datetime")
end)

-- ============================================================================
print("\n=== 15. INDEX: Full indexing integration test ===")
-- ============================================================================

test("index builds and parses datetime fields in frontmatter", function()
  local Index = require("andrew.vault.query.index").Index
  local idx = Index.new(tmp_vault)
  idx:build_sync()

  local page = idx:get_page("Projects/Alpha/Dashboard.md")
  assert_true(page ~= nil, "Dashboard page should be indexed")
  assert_eq(page.type, "project", "type field should be parsed")
  assert_eq(page.status, "active", "status field should be parsed")

  -- Check that modified (a datetime value) was parsed as a Date
  local mod = page.modified
  assert_true(type(mod) == "table", "modified should be parsed as Date table, got " .. type(mod))
  assert_eq(mod.year, 2026, "modified year should be 2026")
  assert_eq(mod.month, 1, "modified month should be 1")
  assert_eq(mod.hour, 10, "modified hour should be 10")
  assert_eq(mod.min, 30, "modified minute should be 30")
end)

test("index skips .obsidian directory", function()
  local Index = require("andrew.vault.query.index").Index
  -- Write a file inside .obsidian
  write_file(".obsidian/app.json", '{"vimMode": true}')
  local idx = Index.new(tmp_vault)
  idx:build_sync()

  local page = idx:get_page(".obsidian/app.json")
  assert_true(page == nil, ".obsidian files should be skipped")
end)

test("index extracts tags from both frontmatter and body", function()
  local Index = require("andrew.vault.query.index").Index
  local idx = Index.new(tmp_vault)
  idx:build_sync()

  local page = idx:get_page("Domains/Physics/Concepts.md")
  assert_true(page ~= nil, "Concepts page should be indexed")

  local tag_set = {}
  for _, tag in ipairs(page.file.tags) do
    tag_set[tag] = true
  end

  assert_true(tag_set["physics"], "Should have 'physics' tag from frontmatter")
  assert_true(tag_set["mechanics"], "Should have 'mechanics' tag from frontmatter and body")
end)

test("index does not extract tags from inside code blocks", function()
  local Index = require("andrew.vault.query.index").Index
  local idx = Index.new(tmp_vault)
  idx:build_sync()

  local page = idx:get_page("Domains/Physics/Concepts.md")
  assert_true(page ~= nil, "Concepts page should be indexed")

  -- The file has [[fake link]] inside a code block - check it's not in outlinks
  local has_fake = false
  for _, link in ipairs(page.file.outlinks) do
    if link.path == "fake link" then
      has_fake = true
    end
  end
  assert_false(has_fake, "Should NOT extract links from inside code blocks")
end)

test("index extracts tasks with inline fields", function()
  local Index = require("andrew.vault.query.index").Index
  local idx = Index.new(tmp_vault)
  idx:build_sync()

  local page = idx:get_page("Projects/Alpha/Tasks/Task1.md")
  assert_true(page ~= nil, "Task1 page should be indexed")
  assert_true(#page.file.tasks > 0, "Should have tasks extracted")

  -- Check task statuses
  local statuses = {}
  for _, task in ipairs(page.file.tasks) do
    statuses[task.status] = true
  end
  assert_true(statuses[" "], "Should have open task")
  assert_true(statuses["x"], "Should have completed task")
  assert_true(statuses["/"], "Should have in-progress task")
  assert_true(statuses["-"], "Should have cancelled task")
  assert_true(statuses[">"], "Should have deferred task")
end)

test("index resolves inlinks correctly", function()
  local Index = require("andrew.vault.query.index").Index
  local idx = Index.new(tmp_vault)
  idx:build_sync()

  -- Beta links to Alpha, so Alpha should have an inlink from Beta
  local alpha_page = idx:get_page("Projects/Alpha/Dashboard.md")
  assert_true(alpha_page ~= nil, "Alpha page should exist")

  local has_beta_inlink = false
  for _, link in ipairs(alpha_page.file.inlinks) do
    if link.path:match("Beta") then
      has_beta_inlink = true
    end
  end
  assert_true(has_beta_inlink, "Alpha should have inlink from Beta (which uses [[Alpha|Alpha Project]])")
end)

-- ============================================================================
print("\n=== 16. EXECUTOR: Functional query tests ===")
-- ============================================================================

test("executor TABLE query with WHERE", function()
  local types = require("andrew.vault.query.types")
  local executor = require("andrew.vault.query.executor")
  local Index = require("andrew.vault.query.index").Index

  local idx = Index.new(tmp_vault)
  idx:build_sync()

  local ast = {
    type = "TABLE",
    fields = {
      { expr = { type = "field", path = { "status" } }, alias = "status" },
    },
    where = {
      type = "binary",
      op = "=",
      left = { type = "field", path = { "type" } },
      right = { type = "literal", value = "project" },
    },
    without_id = false,
  }

  local results, err = executor.execute(ast, idx, "")
  assert_true(err == nil, "Should not error: " .. tostring(err))
  assert_true(#results > 0, "Should have results")
  assert_true(results[1].rows ~= nil, "Should have rows")
  assert_true(#results[1].rows >= 2, "Should find at least 2 project notes")
end)

test("executor TASK query extracts tasks", function()
  local types = require("andrew.vault.query.types")
  local executor = require("andrew.vault.query.executor")
  local Index = require("andrew.vault.query.index").Index

  local idx = Index.new(tmp_vault)
  idx:build_sync()

  local ast = {
    type = "TASK",
    from = { type = "folder", path = "Projects/Alpha/Tasks" },
    without_id = false,
  }

  local results, err = executor.execute(ast, idx, "")
  assert_true(err == nil, "Should not error: " .. tostring(err))
  assert_true(#results > 0, "Should have results")
  assert_true(results[1].type == "task_list", "Should be a task_list result")
  assert_true(#results[1].groups > 0, "Should have task groups")

  local total_tasks = 0
  for _, g in ipairs(results[1].groups) do
    total_tasks = total_tasks + #g.tasks
  end
  assert_true(total_tasks >= 5, "Should find at least 5 tasks, got " .. total_tasks)
end)

-- ============================================================================
print("\n=== 17. TAG_HIGHLIGHTS: Module structure ===")
-- ============================================================================

test("tag_highlights module defines highlight groups", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/tag_highlights.lua"), "r"):read("*a")
  assert_true(src:find("VaultTag") ~= nil, "defines VaultTag highlight group")
  assert_true(src:find("VaultTagProject") ~= nil, "defines VaultTagProject group")
  assert_true(src:find("VaultTagStatus") ~= nil, "defines VaultTagStatus group")
  assert_true(src:find("VaultTagType") ~= nil, "defines VaultTagType group")
  assert_true(src:find("VaultTagPerson") ~= nil, "defines VaultTagPerson group")
  assert_true(src:find("VaultTagHash") ~= nil, "defines VaultTagHash group")
end)

test("tag_highlights uses extmarks", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/tag_highlights.lua"), "r"):read("*a")
  assert_true(src:find("nvim_buf_set_extmark") ~= nil, "uses extmarks")
end)

test("tag_highlights has code block filtering", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/tag_highlights.lua"), "r"):read("*a")
  assert_true(src:find("build_code_exclusion") ~= nil, "has code block filtering")
end)

test("tag_highlights has hex color filtering", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/tag_highlights.lua"), "r"):read("*a")
  assert_true(src:find("is_hex_color") ~= nil, "has hex color filtering")
end)

test("tag_highlights validates tag boundaries", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/tag_highlights.lua"), "r"):read("*a")
  assert_true(src:find("valid_tag_start") ~= nil, "validates tag boundaries")
end)

test("tag_highlights has debounced update", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/tag_highlights.lua"), "r"):read("*a")
  assert_true(src:find("schedule_update") ~= nil, "has debounced update")
end)

test("tag_highlights has tag navigation", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/tag_highlights.lua"), "r"):read("*a")
  assert_true(src:find("jump_tag") ~= nil, "has tag navigation")
end)

test("tag_highlights is registered in init.lua", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/init.lua"), "r"):read("*a")
  assert_true(
    src:find('require("andrew.vault.tag_highlights").setup()', 1, true) ~= nil,
    "tag_highlights should be loaded in init.lua"
  )
end)

test("tag_highlights config section exists", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/config.lua"), "r"):read("*a")
  assert_true(src:find("tag_highlights") ~= nil, "config should have tag_highlights section")
end)

-- ============================================================================
print("\n=== 18. INLINE_FIELDS: Module structure ===")
-- ============================================================================

test("inline_fields module defines highlight groups", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/inline_fields.lua"), "r"):read("*a")
  assert_true(src:find("VaultFieldKey") ~= nil, "defines VaultFieldKey highlight group")
  assert_true(src:find("VaultFieldValue") ~= nil, "defines VaultFieldValue group")
  assert_true(src:find("VaultFieldBracket") ~= nil, "defines VaultFieldBracket group")
  assert_true(src:find("VaultFieldSep") ~= nil, "defines VaultFieldSep group")
  assert_true(src:find("VaultFieldValueDate") ~= nil, "defines VaultFieldValueDate group")
  assert_true(src:find("VaultFieldValueNumber") ~= nil, "defines VaultFieldValueNumber group")
  assert_true(src:find("VaultFieldValueLink") ~= nil, "defines VaultFieldValueLink group")
  assert_true(src:find("VaultFieldValueBool") ~= nil, "defines VaultFieldValueBool group")
end)

test("inline_fields uses extmarks", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/inline_fields.lua"), "r"):read("*a")
  assert_true(src:find("nvim_buf_set_extmark") ~= nil, "uses extmarks")
end)

test("inline_fields has code block filtering", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/inline_fields.lua"), "r"):read("*a")
  assert_true(src:find("build_code_exclusion") ~= nil, "has code block filtering")
end)

test("inline_fields parses all three field syntaxes", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/inline_fields.lua"), "r"):read("*a")
  assert_true(src:find("find_bracket_fields") ~= nil, "parses bracket fields")
  assert_true(src:find("find_paren_fields") ~= nil, "parses paren fields")
  assert_true(src:find("find_standalone_fields") ~= nil, "parses standalone fields")
end)

test("inline_fields classifies value types", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/inline_fields.lua"), "r"):read("*a")
  assert_true(src:find("classify_value") ~= nil, "classifies value types")
end)

test("inline_fields has debounced update", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/inline_fields.lua"), "r"):read("*a")
  assert_true(src:find("schedule_update") ~= nil, "has debounced update")
end)

test("inline_fields has field navigation", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/inline_fields.lua"), "r"):read("*a")
  assert_true(src:find("jump_field") ~= nil, "has field navigation")
end)

test("completion_inline_fields source exists", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/completion_inline_fields.lua"), "r"):read("*a")
  assert_true(src:find("create_source") ~= nil, "uses completion_base.create_source()")
  assert_true(src:find("get_completions") ~= nil, "has get_completions handler")
end)

test("inline_fields is registered in init.lua", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/init.lua"), "r"):read("*a")
  assert_true(
    src:find('require("andrew.vault.inline_fields").setup()', 1, true) ~= nil,
    "inline_fields should be loaded in init.lua"
  )
end)

test("inline_fields config section exists", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/config.lua"), "r"):read("*a")
  assert_true(src:find("inline_fields") ~= nil, "config should have inline_fields section")
end)

-- ============================================================================
print("\n=== 19. INLINE_FIELDS: Parsing correctness ===")
-- ============================================================================

test("bracket field pattern extracts key and value", function()
  local line = "This has [status:: Active] in it"
  local key, value = line:match("%[([%w_%-]+)::%s*(.-)%]")
  assert_eq(key, "status", "bracket field key extracted")
  assert_eq(value, "Active", "bracket field value extracted")
end)

test("paren field pattern extracts key and value", function()
  local line = "Hidden (priority:: 1) metadata"
  local key, value = line:match("%(([%w_%-]+)::%s*(.-)%)")
  assert_eq(key, "priority", "paren field key extracted")
  assert_eq(value, "1", "paren field value extracted")
end)

test("standalone field pattern extracts key and value", function()
  local line = "due:: 2026-03-15"
  local key, value = line:match("^([%w_%-]+)::%s*(.*)")
  assert_eq(key, "due", "standalone field key extracted")
  assert_eq(value, "2026-03-15", "standalone field value extracted")
end)

test("markdown link is not matched as bracket field", function()
  local line = "[click here](https://example.com)"
  local key = line:match("%[([%w_%-]+)::%s*(.-)%]")
  assert_true(key == nil, "markdown link not matched as field")
end)

test("wikilink is not matched as bracket field", function()
  local line = "[[Note Title]]"
  local key = line:match("%[([%w_%-]+)::%s*(.-)%]")
  assert_true(key == nil, "wikilink not matched as field")
end)

test("footnote is not matched as bracket field", function()
  local line = "[^footnote]"
  local key = line:match("%[([%w_%-]+)::%s*(.-)%]")
  assert_true(key == nil, "footnote not matched as field")
end)

test("list item standalone field pattern works", function()
  local line = "- priority:: 2"
  local prefix, key, value = line:match("^(%s*[-*]%s+)([%w_%-]+)::%s*(.*)")
  assert_true(prefix ~= nil, "list prefix matched")
  assert_eq(key, "priority", "list field key extracted")
  assert_eq(value, "2", "list field value extracted")
end)

test("multiple bracket fields on one line", function()
  local line = "[a:: b] and [c:: d]"
  local matches = {}
  local pos = 1
  while pos <= #line do
    local s, e, key, value = line:find("%[([%w_%-]+)::%s*(.-)%]", pos)
    if not s then break end
    matches[#matches + 1] = { key = key, value = value }
    pos = e + 1
  end
  assert_eq(#matches, 2, "should find 2 bracket fields")
  assert_eq(matches[1].key, "a", "first key is 'a'")
  assert_eq(matches[2].key, "c", "second key is 'c'")
end)

test("empty value field is valid", function()
  local line = "[key::]"
  local key, value = line:match("%[([%w_%-]+)::%s*(.-)%]")
  assert_eq(key, "key", "empty value key extracted")
  assert_eq(value, "", "empty value is empty string")
end)

test("hyphenated key is valid", function()
  local line = "[my-key:: value]"
  local key, value = line:match("%[([%w_%-]+)::%s*(.-)%]")
  assert_eq(key, "my-key", "hyphenated key extracted")
  assert_eq(value, "value", "value extracted")
end)

test("underscored key is valid", function()
  local line = "[my_key:: value]"
  local key, value = line:match("%[([%w_%-]+)::%s*(.-)%]")
  assert_eq(key, "my_key", "underscored key extracted")
end)

-- ============================================================================
print("\n=== 20. CALLOUT_FOLDS: Module structure ===")
-- ============================================================================

test("callout_folds module has fingerprint function", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/callout_folds.lua"), "r"):read("*a")
  assert_true(src:find("fingerprint") ~= nil, "has fingerprint function")
end)

test("callout_folds module has record_toggle function", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/callout_folds.lua"), "r"):read("*a")
  assert_true(src:find("record_toggle") ~= nil, "has record_toggle function")
end)

test("callout_folds module has restore function", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/callout_folds.lua"), "r"):read("*a")
  assert_true(src:find("restore") ~= nil, "has restore function")
end)

test("callout_folds module has clear function", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/callout_folds.lua"), "r"):read("*a")
  assert_true(src:find("clear") ~= nil, "has clear function")
end)

test("callout_folds uses json_store for persistence", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/callout_folds.lua"), "r"):read("*a")
  assert_true(src:find("json_store") ~= nil, "uses json_store for persistence")
end)

test("callout_folds uses correct cache filename", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/callout_folds.lua"), "r"):read("*a")
  assert_true(src:find("vault%-callout%-folds") ~= nil, "uses correct cache filename")
end)

test("callout_folds uses sha256 for content hashing", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/callout_folds.lua"), "r"):read("*a")
  assert_true(src:find("sha256") ~= nil, "uses sha256 for content hashing")
end)

test("callout_folds defines VaultFoldClear command", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/callout_folds.lua"), "r"):read("*a")
  assert_true(src:find("VaultFoldClear") ~= nil, "defines VaultFoldClear command")
end)

test("callout_folds has auto-prune logic", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/callout_folds.lua"), "r"):read("*a")
  assert_true(src:find("prune") ~= nil, "has auto-prune logic")
end)

test("callout_folds has default_state function", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/callout_folds.lua"), "r"):read("*a")
  assert_true(src:find("default_state") ~= nil, "has default_state function")
end)

test("callout_folds is registered in init.lua", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/init.lua"), "r"):read("*a")
  assert_true(
    src:find('require("andrew.vault.callout_folds").setup()', 1, true) ~= nil,
    "callout_folds should be loaded in init.lua"
  )
end)

test("callout_folds config section exists", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/config.lua"), "r"):read("*a")
  assert_true(src:find("callout_folds") ~= nil, "config should have callout_folds section")
end)

test("render-markdown integrates callout_folds.record_toggle", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/plugins/render-markdown.lua"), "r"):read("*a")
  assert_true(src:find("record_toggle") ~= nil, "render-markdown calls record_toggle")
end)

test("render-markdown integrates callout_folds.restore", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/plugins/render-markdown.lua"), "r"):read("*a")
  assert_true(src:find("callout_folds%.restore") ~= nil or src:find("callout_folds.restore") ~= nil,
    "render-markdown calls callout_folds.restore")
end)

test("render-markdown switches to manual foldmethod", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/plugins/render-markdown.lua"), "r"):read("*a")
  assert_true(src:find('foldmethod') ~= nil, "render-markdown manages foldmethod")
end)

test("engine invalidates callout_folds cache", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/engine.lua"), "r"):read("*a")
  assert_true(src:find("callout_folds") ~= nil, "engine invalidates callout_folds cache")
end)

-- ============================================================================
-- 21. UNLINKED MENTIONS: module structure
-- ============================================================================

test("unlinked: module has all core functions", function()
  local base = vim.fn.expand("~/.config/nvim/lua/andrew/vault/unlinked")
  local src = io.open(base .. ".lua", "r"):read("*a")
  local rg_src = io.open(base .. "/rg_pipeline.lua", "r"):read("*a")
  local ui_src = io.open(base .. "/ui.lua", "r"):read("*a")
  local wrapper_src = io.open(base .. "/wrapper.lua", "r"):read("*a")
  local utils_src = io.open(base .. "/utils.lua", "r"):read("*a")

  -- orchestrator (unlinked.lua)
  assert_true(src:find("unlinked_mentions") ~= nil, "has per-note unlinked_mentions()")
  assert_true(src:find("vault_unlinked_mentions") ~= nil, "has vault-wide scan")
  assert_true(src:find("rg_pipeline") ~= nil, "requires rg_pipeline submodule")
  assert_true(src:find("filter_results") ~= nil, "delegates result filtering to rg_pipeline")
  assert_true(src:find("names") ~= nil, "requires names submodule")
  assert_true(src:find("ui") ~= nil, "requires ui submodule")
  assert_true(src:find("wrapper") ~= nil or src:find("ui") ~= nil, "delegates wrapping to submodules")
  assert_true(src:find("VaultUnlinked") ~= nil, "defines user command")

  -- rg_pipeline submodule
  assert_true(rg_src:find("overlaps_range") ~= nil, "filters existing links via overlaps_range")
  assert_true(rg_src:find("get_link_ranges") ~= nil, "gets link ranges for filtering")
  assert_true(rg_src:find("rg_escape") ~= nil, "escapes regex special chars")
  assert_true(rg_src:find("\\\\b") ~= nil, "uses word boundaries")

  -- ui submodule
  assert_true(ui_src:find("ctrl%-w") ~= nil, "has wrap action")

  -- wrapper submodule
  assert_true(wrapper_src:find("wrap_in_wikilink") ~= nil, "has wikilink wrapping")

  -- utils submodule
  assert_true(utils_src:find("read_lines_prefer_buffer") ~= nil, "has read_lines_prefer_buffer utility fn")
  assert_true(utils_src:find("filter_by_min_length") ~= nil, "has name length filter")
  assert_true(utils_src:find("group_by_file") ~= nil, "has group_by_file utility")
  assert_true(utils_src:find("batch_list") ~= nil, "has batch_list utility")
  assert_true(utils_src:find("find_name_at_col") ~= nil, "has find_name_at_col utility")
end)

test("unlinked: rg_pattern building", function()
  local ok, rg_pipeline = pcall(require, "andrew.vault.unlinked.rg_pipeline")
  if ok and rg_pipeline._build_rg_pattern then
    local pat = rg_pipeline._build_rg_pattern({"Mesh Convergence", "CFD"})
    -- Longest names first
    assert_true(pat:find("Mesh Convergence") ~= nil, "includes full name")
    assert_true(pat:find("CFD") ~= nil, "includes short name")
    assert_true(pat:find("\\b") ~= nil, "has word boundaries")

    -- Empty names
    local empty = rg_pipeline._build_rg_pattern({})
    assert_true(empty == "", "empty pattern for no names")

    -- Short names filtered
    local short = rg_pipeline._build_rg_pattern({"ab"})
    assert_true(short == "", "filters names shorter than MIN_NAME_LENGTH")
  end
end)

test("unlinked: link/URL detection (unified)", function()
  local ok, link_scan = pcall(require, "andrew.vault.link_scan")
  if ok and link_scan.overlaps_range and link_scan.get_link_ranges then
    local function fn(line, ms, me)
      return link_scan.overlaps_range(ms - 1, me, link_scan.get_link_ranges(line))
    end
    assert_true(fn("see [[Mesh Convergence]] here", 6, 23), "inside wikilink")
    assert_true(not fn("Mesh Convergence is great", 1, 16), "not in wikilink")
    assert_true(fn("[[Mesh Convergence|MC]]", 3, 18), "inside aliased wikilink")
    assert_true(not fn("before [[link]] Mesh Convergence after", 17, 32), "after wikilink")
    -- URL detection (previously separate _is_inside_url)
    local url_line = "see https://example.com/Mesh+Convergence here"
    assert_true(fn(url_line, 24, 39), "inside URL")
    assert_true(not fn(url_line, 1, 3), "before URL")
  end
end)

test("unlinked: code span detection", function()
  local ok, link_scan = pcall(require, "andrew.vault.link_scan")
  if ok and link_scan.is_inside_code_span then
    local fn = link_scan.is_inside_code_span
    assert_true(fn("`Mesh Convergence` here", 2), "inside code span")
    assert_true(not fn("Mesh Convergence `other`", 1), "not in code span")
    assert_true(fn("``Mesh Convergence`` here", 3), "inside double backtick span")
  end
end)

test("unlinked: frontmatter detection", function()
  local ok, link_scan = pcall(require, "andrew.vault.link_scan")
  if ok and link_scan.is_in_frontmatter_lines then
    local fn = link_scan.is_in_frontmatter_lines
    local lines = {"---", "title: Test", "---", "body text"}
    assert_true(fn(lines, 1), "opening --- is frontmatter")
    assert_true(fn(lines, 2), "field line is frontmatter")
    assert_true(fn(lines, 3), "closing --- is frontmatter")
    assert_true(not fn(lines, 4), "body is not frontmatter")
    assert_true(not fn({"no frontmatter"}, 1), "no frontmatter at all")
  end
end)

test("unlinked: fenced code detection", function()
  local ok, link_scan = pcall(require, "andrew.vault.link_scan")
  if ok and link_scan.is_in_fenced_code_lines then
    local fn = link_scan.is_in_fenced_code_lines
    local lines = {"normal", "```python", "code here", "```", "normal again"}
    assert_true(not fn(lines, 1), "before fence")
    assert_true(fn(lines, 3), "inside fence")
    assert_true(not fn(lines, 5), "after fence")
  end
end)

test("unlinked: heading detection", function()
  local ok, link_scan = pcall(require, "andrew.vault.link_scan")
  if ok and link_scan.is_heading_line then
    local fn = link_scan.is_heading_line
    assert_true(fn("# Heading"), "h1 heading")
    assert_true(fn("## Sub Heading"), "h2 heading")
    assert_true(not fn("Not a heading"), "not a heading")
    assert_true(not fn("#tag not heading"), "tag not heading")
  end
end)

test("unlinked: link_scan integration (get_link_ranges)", function()
  local ok, link_scan = pcall(require, "andrew.vault.link_scan")
  if ok then
    local ranges = link_scan.get_link_ranges("see [[Note]] and https://example.com here")
    assert_true(#ranges >= 2, "detects both wikilink and URL ranges")
  end
end)

-- ============================================================================
print("\n=== 24. AUTOLINK: Module structure ===")
-- ============================================================================

test("autolink module defines highlight groups", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/autolink.lua"), "r"):read("*a")
  assert_true(src:find("VaultAutoLinkHint") ~= nil, "defines hint highlight group")
  assert_true(src:find("VaultAutoLinkIcon") ~= nil, "defines icon highlight group")
end)

test("autolink uses extmarks", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/autolink.lua"), "r"):read("*a")
  assert_true(src:find("nvim_buf_set_extmark") ~= nil, "uses extmarks")
end)

test("autolink delegates scanning to link_scan", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/autolink.lua"), "r"):read("*a")
  assert_true(src:find("scan_buffer_names") ~= nil, "delegates to link_scan.scan_buffer_names")
end)

test("link_scan has code block filtering", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/link_scan.lua"), "r"):read("*a")
  assert_true(src:find("build_code_exclusion") ~= nil, "has code block filtering")
end)

test("link_scan has frontmatter filtering", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/link_scan.lua"), "r"):read("*a")
  assert_true(src:find("get_frontmatter_range") ~= nil, "has frontmatter filtering")
end)

test("link_scan has word boundary validation", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/link_scan.lua"), "r"):read("*a")
  assert_true(src:find("has_word_boundaries") ~= nil, "validates word boundaries")
end)

test("autolink has debounced update", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/autolink.lua"), "r"):read("*a")
  assert_true(src:find("schedule_update") ~= nil, "has debounced update")
end)

test("autolink has accept function", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/autolink.lua"), "r"):read("*a")
  assert_true(src:find("M.accept") ~= nil, "has accept function")
end)

test("autolink has toggle function", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/autolink.lua"), "r"):read("*a")
  assert_true(src:find("M.toggle") ~= nil, "has toggle function")
end)

test("autolink checks vault index readiness", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/autolink.lua"), "r"):read("*a")
  assert_true(src:find("index_ready") ~= nil, "checks vault index readiness")
end)

test("autolink debug reports multi-word name count", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/autolink.lua"), "r"):read("*a")
  assert_true(src:find("multi_count") ~= nil, "debug reports multi-word count")
end)

test("autolink debug reports single-word name count", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/autolink.lua"), "r"):read("*a")
  assert_true(src:find("single_count") ~= nil, "debug reports single-word count")
end)

test("autolink is registered in init.lua", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/init.lua"), "r"):read("*a")
  assert_true(
    src:find('require("andrew.vault.autolink").setup()', 1, true) ~= nil,
    "autolink should be loaded in init.lua"
  )
end)

test("autolink config section exists", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/config.lua"), "r"):read("*a")
  assert_true(src:find("M.autolink") ~= nil, "config should have autolink section")
end)

test("engine invalidates autolink cache", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/engine.lua"), "r"):read("*a")
  assert_true(src:find("autolink") ~= nil, "engine invalidates autolink cache")
end)

-- ============================================================================
-- Cleanup
-- ============================================================================

-- Remove temp vault
vim.fn.delete(tmp_vault, "rf")

-- ============================================================================
-- Alias collision detection
-- ============================================================================

print("\n--- Alias collision detection ---")

-- Test: alias collision detection in vault_index
test("vault_index has _detect_collisions method", function()
  local source = io.open("lua/andrew/vault/vault_index.lua", "r")
  assert_true(source ~= nil, "could not open vault_index.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("function M.VaultIndex:_detect_collisions") ~= nil,
    "has _detect_collisions method")
end)

test("vault_index has _notify_collisions method", function()
  local source = io.open("lua/andrew/vault/vault_index.lua", "r")
  assert_true(source ~= nil, "could not open vault_index.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("function M.VaultIndex:_notify_collisions") ~= nil,
    "has _notify_collisions method")
end)

test("vault_index has get_collisions query method", function()
  local source = io.open("lua/andrew/vault/vault_index.lua", "r")
  assert_true(source ~= nil, "could not open vault_index.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("function M.VaultIndex:get_collisions") ~= nil,
    "has get_collisions query method")
end)

test("vault_index has show_collisions method", function()
  local source = io.open("lua/andrew/vault/vault_index.lua", "r")
  assert_true(source ~= nil, "could not open vault_index.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("function M.VaultIndex:show_collisions") ~= nil,
    "has show_collisions method")
end)

test("_rebuild_name_index calls _detect_collisions", function()
  local source = io.open("lua/andrew/vault/vault_index.lua", "r")
  assert_true(source ~= nil, "could not open vault_index.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("self:_detect_collisions%(name_idx, alias_idx%)") ~= nil,
    "_rebuild_name_index calls _detect_collisions")
end)

test("vault_index has _collisions field", function()
  local source = io.open("lua/andrew/vault/vault_index.lua", "r")
  assert_true(source ~= nil, "could not open vault_index.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("_collisions") ~= nil,
    "has _collisions field")
end)

test("detects alias-alias collisions", function()
  local source = io.open("lua/andrew/vault/vault_index.lua", "r")
  assert_true(source ~= nil, "could not open vault_index.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find('"alias%-alias"') ~= nil,
    "detects alias-alias collisions")
end)

test("detects name-alias collisions", function()
  local source = io.open("lua/andrew/vault/vault_index.lua", "r")
  assert_true(source ~= nil, "could not open vault_index.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find('"name%-alias"') ~= nil,
    "detects name-alias collisions")
end)

test("detects basename collisions", function()
  local source = io.open("lua/andrew/vault/vault_index.lua", "r")
  assert_true(source ~= nil, "could not open vault_index.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find('"basename"') ~= nil,
    "detects basename collisions")
end)

test("checks warn_collisions config", function()
  local source = io.open("lua/andrew/vault/vault_index.lua", "r")
  assert_true(source ~= nil, "could not open vault_index.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("warn_collisions") ~= nil,
    "checks warn_collisions config")
end)

test("config has warn_collisions setting", function()
  local source = io.open("lua/andrew/vault/config.lua", "r")
  assert_true(source ~= nil, "could not open config.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("warn_collisions") ~= nil,
    "config has warn_collisions setting")
end)

-- ============================================================================
-- Feature 38: Relative path resolution for wikilinks
-- ============================================================================

print("\n--- Feature 38: Relative path embeds / wikilinks ---")

test("wikilinks has is_path_like() function", function()
  local source = io.open("lua/andrew/vault/wikilinks.lua", "r")
  assert_true(source ~= nil, "could not open wikilinks.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("is_path_like") ~= nil,
    "has is_path_like() path detection function")
end)

test("wikilinks has resolve_relative() function", function()
  local source = io.open("lua/andrew/vault/wikilinks.lua", "r")
  assert_true(source ~= nil, "could not open wikilinks.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("resolve_relative") ~= nil,
    "has resolve_relative() filesystem resolution function")
end)

test("resolve_link checks path-like names before vault index", function()
  local source = io.open("lua/andrew/vault/wikilinks.lua", "r")
  assert_true(source ~= nil, "could not open wikilinks.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("is_path_like.-resolve_relative") ~= nil,
    "resolve_link checks path-like names before vault index")
end)

test("uses vim.fs.normalize for path normalization", function()
  local source = io.open("lua/andrew/vault/wikilinks.lua", "r")
  assert_true(source ~= nil, "could not open wikilinks.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("vim.fs.normalize") ~= nil,
    "uses vim.fs.normalize for ../. resolution")
end)

test("probes with .md extension for extensionless links", function()
  local source = io.open("lua/andrew/vault/wikilinks.lua", "r")
  assert_true(source ~= nil, "could not open wikilinks.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find('%.md"') ~= nil or content:find("%.md$") ~= nil,
    "probes with .md extension for extensionless links")
end)

test("uses vault_path for folder-qualified fallback", function()
  local source = io.open("lua/andrew/vault/wikilinks.lua", "r")
  assert_true(source ~= nil, "could not open wikilinks.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("vault_path") ~= nil,
    "uses vault_path for folder-qualified fallback")
end)

test("new note creation handles path-like links", function()
  local source = io.open("lua/andrew/vault/wikilinks.lua", "r")
  assert_true(source ~= nil, "could not open wikilinks.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("is_path_like.-link") ~= nil,
    "new note creation handles path-like links")
end)

test("parse_target preserves ./ prefix", function()
  local link_utils = require("andrew.vault.link_utils")

  local r1 = link_utils.parse_target("./SiblingNote")
  assert_eq(r1.name, "./SiblingNote", "parse_target preserves ./ prefix")
end)

test("parse_target preserves ../ prefix", function()
  local link_utils = require("andrew.vault.link_utils")

  local r2 = link_utils.parse_target("../Parent/Note")
  assert_eq(r2.name, "../Parent/Note", "parse_target preserves ../ prefix")
end)

test("parse_target preserves folder/ path", function()
  local link_utils = require("andrew.vault.link_utils")

  local r3 = link_utils.parse_target("Subfolder/Note")
  assert_eq(r3.name, "Subfolder/Note", "parse_target preserves folder/ path")
end)

test("parse_target extracts heading from relative path link", function()
  local link_utils = require("andrew.vault.link_utils")

  local r4 = link_utils.parse_target("./Note#Heading")
  assert_eq(r4.name, "./Note", "parse_target extracts name from relative path with heading")
  assert_eq(r4.heading, "Heading", "parse_target extracts heading from relative path link")
end)

test("parse_target extracts block_id from relative path link", function()
  local link_utils = require("andrew.vault.link_utils")

  local r5 = link_utils.parse_target("../Dir/Note^blk-123")
  assert_eq(r5.name, "../Dir/Note", "parse_target extracts name from relative path with block")
  assert_eq(r5.block_id, "blk-123", "parse_target extracts block_id from relative path link")
end)

test("parse_target extracts alias from relative path link", function()
  local link_utils = require("andrew.vault.link_utils")

  local r6 = link_utils.parse_target("./Note|Display")
  assert_eq(r6.name, "./Note", "parse_target extracts name from relative path with alias")
  assert_eq(r6.alias, "Display", "parse_target extracts alias from relative path link")
end)

-- ============================================================================
print("\n=== 39. VAULT_STATS: Module structure ===")
-- ============================================================================

test("stats module has compute function", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("function M%.compute") ~= nil, "has compute function")
end)

test("stats module has format function", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("function M%.format") ~= nil, "has format function")
end)

test("stats module has show function", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("function M%.show") ~= nil, "has show function")
end)

test("stats module has setup function", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("function M%.setup") ~= nil, "has setup function")
end)

test("stats module defines VaultStats command", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("VaultStats") ~= nil, "defines VaultStats command")
end)

test("stats module uses vault_index", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("vault_index") ~= nil, "uses vault_index")
end)

test("stats module uses float display", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("create_float_display") ~= nil, "uses float display")
end)

test("stats module computes orphans", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("orphan") ~= nil, "computes orphans")
end)

test("stats module computes broken links", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("broken") ~= nil, "computes broken links")
end)

test("stats module uses resolve_name for broken link detection", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("resolve_name") ~= nil, "uses resolve_name for broken link detection")
end)

test("stats module computes most connected notes", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("top_connected") ~= nil, "computes most connected")
end)

test("stats module computes tag distribution", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("tag_counts") ~= nil, "computes tag distribution")
end)

test("stats module computes type distribution", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("type_counts") ~= nil, "computes type distribution")
end)

test("stats module computes monthly activity", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("month_counts") ~= nil, "computes monthly activity")
end)

test("stats module computes task summary", function()
  local source = io.open("lua/andrew/vault/stats.lua", "r")
  assert_true(source ~= nil, "could not open stats.lua")
  local content = source:read("*a")
  source:close()

  assert_true(content:find("task_counts") ~= nil, "computes task summary")
end)

test("stats module is registered in init.lua", function()
  local src = io.open(vim.fn.expand("~/.config/nvim/lua/andrew/vault/init.lua"), "r"):read("*a")
  assert_true(
    src:find('require("andrew.vault.stats").setup()', 1, true) ~= nil,
    "stats should be loaded in init.lua"
  )
end)

-- ============================================================================
-- Feature 40: Temporal alias weekday resolution
-- ============================================================================

print("\n--- Feature 40: Temporal alias weekday resolution ---")

-- Replicate the exact weekday math from wikilinks.lua resolve_temporal() so we
-- can verify correctness for every combination without needing a live vault.
-- WEEKDAYS mirrors the local table in wikilinks.lua (Sunday=1 .. Saturday=7).
local WEEKDAYS = {
  sunday = 1, monday = 2, tuesday = 3, wednesday = 4,
  thursday = 5, friday = 6, saturday = 7,
}

--- Pure reimplementation of the weekday-diff math from resolve_temporal().
--- @param direction string "last" or "next"
--- @param target_wday number  1-7 (Sunday=1)
--- @param today_wday  number  1-7 (Sunday=1)
--- @return number  signed day offset from today
local function weekday_diff(direction, target_wday, today_wday)
  local diff
  if direction == "last" then
    diff = today_wday - target_wday
    if diff <= 0 then diff = diff + 7 end
    diff = -diff
  else -- "next"
    diff = target_wday - today_wday
    if diff <= 0 then diff = diff + 7 end
  end
  return diff
end

-- ----- Algorithm unit tests (pure math, no module deps) ---------------------

test("last monday on a Monday -> -7 (previous week)", function()
  -- Monday = wday 2
  local diff = weekday_diff("last", WEEKDAYS.monday, WEEKDAYS.monday)
  assert_eq(diff, -7, "last monday on monday")
end)

test("next monday on a Monday -> +7 (next week)", function()
  local diff = weekday_diff("next", WEEKDAYS.monday, WEEKDAYS.monday)
  assert_eq(diff, 7, "next monday on monday")
end)

test("last tuesday on a Monday -> -6", function()
  -- Monday(2) - Tuesday(3) = -1; -1 <= 0 -> +7 = 6; negate = -6
  local diff = weekday_diff("last", WEEKDAYS.tuesday, WEEKDAYS.monday)
  assert_eq(diff, -6, "last tuesday on monday")
end)

test("next tuesday on a Monday -> +1", function()
  -- Tuesday(3) - Monday(2) = 1; 1 > 0 -> stays 1
  local diff = weekday_diff("next", WEEKDAYS.tuesday, WEEKDAYS.monday)
  assert_eq(diff, 1, "next tuesday on monday")
end)

test("last friday on a Monday -> -3", function()
  -- Monday(2) - Friday(6) = -4; -4 <= 0 -> +7 = 3; negate = -3
  local diff = weekday_diff("last", WEEKDAYS.friday, WEEKDAYS.monday)
  assert_eq(diff, -3, "last friday on monday")
end)

test("next friday on a Monday -> +4", function()
  -- Friday(6) - Monday(2) = 4; 4 > 0 -> stays 4
  local diff = weekday_diff("next", WEEKDAYS.friday, WEEKDAYS.monday)
  assert_eq(diff, 4, "next friday on monday")
end)

test("last sunday on a Wednesday -> -3", function()
  -- Wednesday(4) - Sunday(1) = 3; 3 > 0 -> stays 3; negate = -3
  local diff = weekday_diff("last", WEEKDAYS.sunday, WEEKDAYS.wednesday)
  assert_eq(diff, -3, "last sunday on wednesday")
end)

test("next sunday on a Wednesday -> +4", function()
  -- Sunday(1) - Wednesday(4) = -3; -3 <= 0 -> +7 = 4
  local diff = weekday_diff("next", WEEKDAYS.sunday, WEEKDAYS.wednesday)
  assert_eq(diff, 4, "next sunday on wednesday")
end)

test("last saturday on a Saturday -> -7", function()
  local diff = weekday_diff("last", WEEKDAYS.saturday, WEEKDAYS.saturday)
  assert_eq(diff, -7, "last saturday on saturday (same day = previous week)")
end)

test("next saturday on a Saturday -> +7", function()
  local diff = weekday_diff("next", WEEKDAYS.saturday, WEEKDAYS.saturday)
  assert_eq(diff, 7, "next saturday on saturday (same day = next week)")
end)

test("last wednesday on a Friday -> -2", function()
  -- Friday(6) - Wednesday(4) = 2; 2 > 0 -> stays 2; negate = -2
  local diff = weekday_diff("last", WEEKDAYS.wednesday, WEEKDAYS.friday)
  assert_eq(diff, -2, "last wednesday on friday")
end)

test("next wednesday on a Friday -> +5", function()
  -- Wednesday(4) - Friday(6) = -2; -2 <= 0 -> +7 = 5
  local diff = weekday_diff("next", WEEKDAYS.wednesday, WEEKDAYS.friday)
  assert_eq(diff, 5, "next wednesday on friday")
end)

-- ----- Integration tests through resolve_link (module-level) ----------------
-- resolve_link falls back to resolve_temporal when vault_index.current() is nil
-- (no vault index initialized). We temporarily set engine.vault_path to a temp
-- dir so the returned paths are predictable.

-- Helper: compute expected date string for a day offset from today
local function expected_date(offset)
  local date_utils = require("andrew.vault.date_utils")
  return os.date("%Y-%m-%d", os.time() + offset * date_utils.SECS_PER_DAY)
end

test("resolve_link: 'today' resolves to today's daily log", function()
  local engine = require("andrew.vault.engine")
  local cfg = require("andrew.vault.config")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  engine.vault_path = "/tmp/test-temporal-vault"

  local path = wikilinks.resolve_link("today")
  engine.vault_path = saved_vault

  assert_true(path ~= nil, "today should resolve")
  local date = expected_date(0)
  assert_eq(path, "/tmp/test-temporal-vault/" .. cfg.dirs.log .. "/" .. date .. ".md",
    "today path")
end)

test("resolve_link: 'yesterday' resolves to yesterday's daily log", function()
  local engine = require("andrew.vault.engine")
  local cfg = require("andrew.vault.config")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  engine.vault_path = "/tmp/test-temporal-vault"

  local path = wikilinks.resolve_link("yesterday")
  engine.vault_path = saved_vault

  assert_true(path ~= nil, "yesterday should resolve")
  local date = expected_date(-1)
  assert_eq(path, "/tmp/test-temporal-vault/" .. cfg.dirs.log .. "/" .. date .. ".md",
    "yesterday path")
end)

test("resolve_link: 'tomorrow' resolves to tomorrow's daily log", function()
  local engine = require("andrew.vault.engine")
  local cfg = require("andrew.vault.config")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  engine.vault_path = "/tmp/test-temporal-vault"

  local path = wikilinks.resolve_link("tomorrow")
  engine.vault_path = saved_vault

  assert_true(path ~= nil, "tomorrow should resolve")
  local date = expected_date(1)
  assert_eq(path, "/tmp/test-temporal-vault/" .. cfg.dirs.log .. "/" .. date .. ".md",
    "tomorrow path")
end)

test("resolve_link: 'Last Monday' is case-insensitive", function()
  local engine = require("andrew.vault.engine")
  local cfg = require("andrew.vault.config")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  engine.vault_path = "/tmp/test-temporal-vault"

  local path_lower = wikilinks.resolve_link("last monday")
  local path_mixed = wikilinks.resolve_link("Last Monday")
  local path_upper = wikilinks.resolve_link("LAST MONDAY")
  engine.vault_path = saved_vault

  assert_true(path_lower ~= nil, "last monday should resolve")
  assert_eq(path_lower, path_mixed, "case insensitive: lower vs mixed")
  assert_eq(path_lower, path_upper, "case insensitive: lower vs upper")
end)

test("resolve_link: 'last blurgsday' returns nil (invalid weekday)", function()
  local engine = require("andrew.vault.engine")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  engine.vault_path = "/tmp/test-temporal-vault"

  local path = wikilinks.resolve_link("last blurgsday")
  engine.vault_path = saved_vault

  assert_eq(path, nil, "invalid weekday should return nil")
end)

test("resolve_link: temporal aliases disabled returns nil", function()
  local engine = require("andrew.vault.engine")
  local cfg = require("andrew.vault.config")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  local saved_enabled = cfg.temporal_aliases.enabled
  engine.vault_path = "/tmp/test-temporal-vault"
  cfg.temporal_aliases.enabled = false

  local path = wikilinks.resolve_link("today")
  engine.vault_path = saved_vault
  cfg.temporal_aliases.enabled = saved_enabled

  assert_eq(path, nil, "disabled temporal aliases should return nil")
end)

test("resolve_link: 'next friday' resolves to a date in the future", function()
  local engine = require("andrew.vault.engine")
  local cfg = require("andrew.vault.config")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  engine.vault_path = "/tmp/test-temporal-vault"

  local path = wikilinks.resolve_link("next friday")
  engine.vault_path = saved_vault

  assert_true(path ~= nil, "next friday should resolve")
  -- Extract the date portion from the path and verify it's in the future
  local date_str = path:match("(%d%d%d%d%-%d%d%-%d%d)%.md$")
  assert_true(date_str ~= nil, "path should contain a date")
  local today = os.date("%Y-%m-%d")
  assert_true(date_str > today, "next friday date (" .. date_str .. ") should be after today (" .. today .. ")")
end)

test("resolve_link: 'last wednesday' resolves to a date in the past", function()
  local engine = require("andrew.vault.engine")
  local cfg = require("andrew.vault.config")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  engine.vault_path = "/tmp/test-temporal-vault"

  local path = wikilinks.resolve_link("last wednesday")
  engine.vault_path = saved_vault

  assert_true(path ~= nil, "last wednesday should resolve")
  local date_str = path:match("(%d%d%d%d%-%d%d%-%d%d)%.md$")
  assert_true(date_str ~= nil, "path should contain a date")
  local today = os.date("%Y-%m-%d")
  assert_true(date_str < today, "last wednesday date (" .. date_str .. ") should be before today (" .. today .. ")")
end)

test("resolve_link: relative weekdays disabled returns nil for weekday aliases", function()
  local engine = require("andrew.vault.engine")
  local cfg = require("andrew.vault.config")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  local saved_rel = cfg.temporal_aliases.relative_weekdays
  engine.vault_path = "/tmp/test-temporal-vault"
  cfg.temporal_aliases.relative_weekdays = false

  local path_weekday = wikilinks.resolve_link("next monday")
  -- Static aliases should still work
  local path_today = wikilinks.resolve_link("today")

  engine.vault_path = saved_vault
  cfg.temporal_aliases.relative_weekdays = saved_rel

  assert_eq(path_weekday, nil, "relative weekdays disabled -> nil for 'next monday'")
  assert_true(path_today ~= nil, "static alias 'today' should still work")
end)

test("resolve_link: resolved weekday date actually falls on correct weekday", function()
  local engine = require("andrew.vault.engine")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  engine.vault_path = "/tmp/test-temporal-vault"

  -- "next thursday" should resolve to a date whose os.date("%A") is "Thursday"
  local path = wikilinks.resolve_link("next thursday")
  engine.vault_path = saved_vault

  assert_true(path ~= nil, "next thursday should resolve")
  local y, m, d = path:match("(%d%d%d%d)%-(%d%d)%-(%d%d)%.md$")
  assert_true(y ~= nil, "path should contain date components")
  local ts = os.time({ year = tonumber(y), month = tonumber(m), day = tonumber(d), hour = 12 })
  local weekday_name = os.date("%A", ts)
  assert_eq(weekday_name, "Thursday",
    "next thursday resolved to " .. y .. "-" .. m .. "-" .. d .. " which is " .. weekday_name)
end)

test("resolve_link: 'last sunday' resolved date is actually a Sunday", function()
  local engine = require("andrew.vault.engine")
  local wikilinks = require("andrew.vault.wikilinks")

  local saved_vault = engine.vault_path
  engine.vault_path = "/tmp/test-temporal-vault"

  local path = wikilinks.resolve_link("last sunday")
  engine.vault_path = saved_vault

  assert_true(path ~= nil, "last sunday should resolve")
  local y, m, d = path:match("(%d%d%d%d)%-(%d%d)%-(%d%d)%.md$")
  assert_true(y ~= nil, "path should contain date components")
  local ts = os.time({ year = tonumber(y), month = tonumber(m), day = tonumber(d), hour = 12 })
  local weekday_name = os.date("%A", ts)
  assert_eq(weekday_name, "Sunday",
    "last sunday resolved to " .. y .. "-" .. m .. "-" .. d .. " which is " .. weekday_name)
end)

-- ============================================================================
-- Summary
-- ============================================================================

print("\n" .. string.rep("=", 60))
print(string.format("Results: %d passed, %d failed, %d total", passed, failed, passed + failed))
print(string.rep("=", 60))

if #errors > 0 then
  print("\nFailed tests:")
  for _, e in ipairs(errors) do
    print("  - " .. e.name .. ": " .. e.err)
  end
end

-- Exit with appropriate code
vim.cmd("cquit " .. (failed > 0 and "1" or "0"))
